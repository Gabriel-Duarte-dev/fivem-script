...
Citizen.CreateThread(function()
	TriggerServerEvent("sks_spotify:GetDate")
end)
...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "play" then
		if xSound:soundExists(nameid) then
			if xSound:isPaused(nameid) then
				TriggerServerEvent("sks_spotify:ChangeState", true, nameid)
				local esperar = 0
				while nuiaberto do
					Wait(1000)
	...
	elseif data.action == "pause" then
		if xSound:soundExists(nameid) then
			if xSound:isPlaying(nameid) then
				TriggerServerEvent("sks_spotify:ChangeState", false, nameid)
			end
		end
	...
	elseif data.action == "loop" then
		if xSound:soundExists(nameid) then
			...
			TriggerServerEvent("sks_spotify:ChangeLoop",nameid,datasoundinfo.loop)
		else
	...
	elseif data.action == "forward" then
		if xSound:soundExists(nameid) then
			TriggerServerEvent("sks_spotify:ChangePosition", 10, nameid)
		end
...
function ApplySound(quanti,plate)
	...
	if vadi <= 1.01 and vadi >= -0.001 and exis then
		...
		TriggerServerEvent("sks_spotify:ChangeVolume", quanti, plate)
	end
end
...
function SetUrl(url,nid)
	...
	if url then
		...
		if encontrad then
			...
			TriggerServerEvent("sks_spotify:ModifyURL",vehdata)
		else
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				local veh = GetVehiclePedIsIn(PlayerPedId(),false)
				...
				vehdata.loop = datasoundinfo.loop
				TriggerServerEvent("sks_spotify:AddVehicle",vehdata)
			end
		end
	else
...
##             BASE PROGRAMADA POR: SkipS#1234               ##
############################################################### ]]
RegisterNetEvent("sks_spotify:ShowNui")
	AddEventHandler("sks_spotify:ShowNui", function()
	show()
end)
...
AddEventHandler("sks_spotify:AddVehicle", function(data)
...
AddEventHandler("sks_spotify:ChangeState", function(tipo, nome)
...
AddEventHandler("sks_spotify:ChangeLoop", function(tipo, nome)
	if xSound:soundExists(nome) then
...
AddEventHandler("sks_spotify:ChangeVolume", function(som, range, nome)
	local carroe
	local crds
    for i = 1, #Zones do
        local v = Zones[i]
...
##             BASE PROGRAMADA POR: SkipS#1234               ##
############################################################### ]]
RegisterNetEvent("sks_spotify:SendData")
AddEventHandler("sks_spotify:SendData", function(data)
    Zones = data
    for i = 1, #Zones do
		local v = Zones[i]
		if v.isplaying then
